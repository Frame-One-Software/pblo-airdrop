...
contract PBLOAirdrop {
    ...
    constructor(IERC20 _pbloToken) public {
        pbloToken = _pbloToken;
        owner = msg.sender;
    }

    modifier onlyOwner() {
        require(owner == msg.sender, "error : unauthorised");
        _;
    }

    event MultiIERC20Transfer(
        address _from,
        address _to,
        uint _amount,
        IERC20 _token
    ...
    (
        ...
        require(_addresses.length == _amounts.length, "error : invalid input");

        for (uint i = 0; i < _addresses.length; i++) {
            address to = _addresses[i];
            uint256 amount = _amounts[i];
            
            pbloToken.safeTransferFrom(owner, to, amount);

            emit MultiIERC20Transfer(msg.sender, to, amount, pbloToken);
        }
    }
